...
- **The license ships in the built distribution.** `pyproject.toml` declares `license = "Apache-2.0"` and `license-files = ["LICENSE", "hear_embed/_vendor/LICENSE.apache-2.0"]`, so both the project license (root `LICENSE`) and Google's vendored one land as `License-File` entries in the wheel METADATA. `tests/test_packaging.py` guards the source-tree side; the CI `build` job checks the built wheel. The root `LICENSE` is the canonical Apache-2.0 text (byte-copied from the vendored file) — don't truncate it.

## CI / tooling notes

- Three jobs: **lint-and-test** runs `uv sync --locked --no-group model` (torch-free tests + ruff, no torch); **typecheck** runs `uv sync --locked` *with* the model group because `ty` needs torch/transformers importable to check `embedder.py`; **build** runs `uv build`, installs the wheel into a fresh env *without* the model group, and smoke-tests `hear-embed --help` + torch-free imports — validating the "installs and runs torch-free" promise at the distribution level, which the source-tree tests never exercise. On Linux, `[tool.uv]` in `pyproject.toml` resolves CPU-only torch from the `pytorch-cpu` index (hundreds of MB) instead of the multi-GB CUDA build.
